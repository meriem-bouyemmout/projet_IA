...
def classify_iris(sepal_length, sepal_width, petal_length, petal_width):
    ...
    result_window.title("Résultat de classification")

    # Créer un label pour afficher le résultat
    result_label = tk.Label(result_window, text=f"La classification de l'iris saisie est : {prediction}")
    result_label.place(x=500, y=50)
    
    evaluation_label = tk.Label(result_window, text=f"Évaluation du modèle : {evaluation}")
    evaluation_label.place(x=500, y=150)
    classification_label = tk.Label(result_window, text=f"Rapport de classification :\n{classification_rep}")
    classification_label.place(x=500, y=250)
...
fig = Figure(figsize=(5, 5.5), dpi=80)
